...
| 4 | The foundation is progressive context | Introduce the context architecture. The agent receives the smallest useful layer, then expands only when the task requires it. | Main architecture diagram: task at centre, surrounded by repository guidance, knowledge, tools, and targeted code. |
...
| 6 | Knowledge is more than code search | Explain three knowledge levels: repo-specific conventions, platform/org constraints, and curated prior learning. | Three stacked cards. Example: a knowledge system that captures useful session learnings, curates them, and makes them retrievable later. |
| 7 | Live tools complete the context | Code tells you what was written. Tools tell you what is happening now: logs, metrics, build status, deployment state. | Raw systems → task-shaped tool → decision-ready evidence. Use generic labels only. |
...
- When describing the knowledge system, make the quality boundary clear: capture is not truth; curation, deduplication, freshness, and retrieval are part of the system.
