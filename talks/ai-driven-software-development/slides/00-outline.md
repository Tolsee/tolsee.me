...
| 6 | Knowledge should compound, not accumulate | A Linktree example: reusable knowledge is routed to repository guidance for local truth, or to a shared system for cross-repository constraints and learnings. | Engineering work → Linktree knowledge system → in repository / across organisation → future task retrieves the relevant layer. |
